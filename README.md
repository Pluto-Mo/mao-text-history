...
《正处》只是样例。全书研究先读 [公开资料总览](research/maoxuan/README.md)、[逐篇总表](research/maoxuan/all-works.md) 和 [映射规则](research/maoxuan/RULES.md)。229篇正文与1篇附录均有首轮研究卡；全库候选、已复核来源陈述及缺稿分别登记。卡片齐全不等于全部历史稿本完成校勘；当前研究进度见 `research/maoxuan/coverage.json`，资料入库进度仍见 `reports/completion.json`。
...
**全文复制状态以最新 reports/completion.json 为准；全部中间稿的历史校勘不因文件入库而自动完成。**本公开仓库保存原文件、全文、页面索引、来源书目、采集收据、候选映射和来源资料卡。可访问的网页不等于原书全文取得或获准转载。
...
示例：“按全《毛选》篇目汇录公开来源已披露的成文、修改和发表记录，每条给出来源、版次和定位；并列不同说法，不加入助手解释。范围与未完成项以最新报告为准。”
