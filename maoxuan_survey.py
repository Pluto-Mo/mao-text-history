...
def source(rec,kind,number,end=None):
 return {'witness_id':rec['id'],'source_url':rec['url'],'raw_sha256':rec['raw_sha256'],'edition_refs':rec.get('edition_refs',[]),'kind':kind,'number':number,'end':end or number,'record_path':'archive/records/'+rec['id']+'.json','text_path':rec.get('text_path'),'source_title':rec.get('title'),'source_media':rec.get('media'),'source_layer':'official_reader_text' if rec.get('media')=='official_page_text' else ('web_transcription' if rec.get('media')=='html' else 'digital_PDF_copy'),'edition_verification_status':rec.get('edition_verification_status','not_independently_verified'),'public_url_rechecked_this_run':False}
def source_link(s):
...
def build():
 ...
 for item in reviewed['evidence']:
  ...
  for pointer in item.pop('sources'):
   ...
   out=source(rec,pointer['kind'],pointer['number'],pointer.get('end'));out.update(pointer);out['excerpt_text_sha256']=sha(text);out['stored_excerpt_sha256']=sha(text+'\n');out['evidence_text_file']='evidence-pages/'+item['id']+'-'+rec['id']+'-'+pointer['kind']+'-'+str(pointer['number'])+'-'+str(pointer.get('end',pointer['number']))+'.txt'
 ...
 for w in works:
  ...
  w['research_status']='source_records_with_open_verification_items' if ev else 'full_corpus_retrieval_first_pass_only'
  ...
  first=w['opening_note_candidate'];abstract='；'.join(x['claim'].rstrip('。') for x in ev[:3])+'。' if ev else '基线与候选定位已登记；尚未形成逐条来源记载摘要。'
  w['first_pass_summary']=abstract
  md=[f"# {w['id']} · {w['title']}",'',f"卷 {w['volume']} · {w['entry_type']} · 版本所署日期：{w['date_label']}",'','## 来源记载摘要','',abstract,'','本卡汇录公开来源所载信息；摘要是署明来源的转述，不是逐字引文，也不是助手的历史解释。','','## 基线与来源','',source_link(w['baseline']),'',f"读取范围字符数（含题注等）：{w['baseline_text_chars']}；范围SHA256：`{w['baseline_span_text_sha256']}`。",'','## 待核题注片段（自动提取，非已核引文）','',first or '此页未检出独立星号题注；不能据此断定没有编者说明。','',w['opening_note_warning'],'','## 来源记录与原文位置','']
  for e in ev:md += [f"### {e['id']} · {e['relation_type']}",'',e['claim'],'',f"日期／阶段：{e.get('date','未确定')}；归因类别：{e['evidence_class']}。",'',f"证据限制：{e['limit']}",'']+[source_link(s) for s in e['sources']]+['']
 ...
 table=['# 全《毛选》逐篇梳理总表','','229篇正文与1篇附录分别计数；下表全部有基线定位与跨库检索结果。未复核列不等于未修改。','', '| ID | 卷 | 篇名 | 日期标签 | 全库候选 | 复核陈述 | 来源记载／登记状态 |','|---|---:|---|---|---:|---:|---|']
 ...
 checks={'registry_main_count':sum(w['entry_type']=='main_work' for w in works)==229,'one_appendix':sum(w['entry_type']=='appendix' for w in works)==1,'unique_ids':len(workby)==len(works),'all_baselines_nonempty':all(w['baseline_text_chars']>0 for w in works),'all_title_starts_checked':all(w['title_start_checked'] for w in works),'all_cards_and_mapping_files_written':all((OUT/('articles/'+w['id']+'.md')).exists() and (OUT/('mappings/'+w['id']+'.jsonl')).exists() for w in works),'evidence_has_sources':all(e['sources'] for e in evidence),'all_records_are_attributed_paraphrases':all(e.get('summary_type')=='attributed_source_paraphrase' and e.get('assistant_analysis') is None for e in evidence),'evidence_snapshots_match_hashes':all(sha((OUT/s['evidence_text_file']).read_text(encoding='utf-8'))==s['stored_excerpt_sha256'] for e in evidence for s in e['sources']),'no_auto_manuscript_authentication':report['new_manuscripts_authenticated']==0,'fifth_alternate_not_double_counted':not any(w['title']=='讲话稿 按此' for w in works),'text_matches_are_retained':sum(c['id'].startswith('C-') for c in candidates)>len(works),'each_baseline_has_title_candidate':all(any(c['witness_id']==w['baseline']['witness_id'] and c['id'].startswith('C-') for c in grouped[w['id']]) for w in works)}
